...
//Acá reemplazamos el useState nativo por el hook useForm, usando handleChange y 
//handleArrayChange para limpiar significativamente la lógica interna del componente.
...
const Formulario = ({ onSuccess }: FormularioProps) => {
  ...
  const { agregar, editar, participanteActivo } = context;
  ...
  // PARTE 3: Inicializamos nuestro Custom Hook useForm
  ...
  // Controlamos los cambios del participante activo usando el reset de nuestro hook
  useEffect(() => {
    if (participanteActivo) {
      resetForm({ ...participanteActivo, edad: participanteActivo.edad });
    } else {
      resetForm({ nombre: '', email: '', edad: '', pais: 'Argentina', modalidad: 'Presencial', tecnologias: [], nivel: 'Principiante', aceptaTerminos: false });
    }
  }, [participanteActivo]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const p = new Participante(
      ...
      formData.nombre, formData.email, Number(formData.edad), formData.pais, formData.modalidad, formData.tecnologias, formData.nivel, formData.aceptaTerminos
    );
              ...
              return (
                <label key={tech} htmlFor={currentId} className="flex items-center gap-2 font-medium cursor-pointer">
                  <input 
                    ...
                    onChange={() => handleArrayChange('tecnologias', tech)} // PARTE 3: Manejador de array del hook
